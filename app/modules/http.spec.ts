...
describe('HTTP транспорт', () => {
    ...
    beforeEach(() => {
        http = new HTTP('');
        sandbox
            .stub(http, 'p_request' as keyof typeof http)
            .callsFake((...args: any) => {
                const { method } = args[1];
    ...
    it('должен вернуть строку с параметрами при входных всех строковых параметрах при GET запросе', () => {
        http.get('', { data: { a: '1', b: '2' } });
        expect(result).contain('?a=1&b=2').contain('GET');
    });

    it('должен вернуть строку с параметрами при входных параметрах: числовой и строка при GET запросе', () => {
    ...
    it('должен кодировать строку специальные символы для передачи в параметрах при GET запросе', () => {
